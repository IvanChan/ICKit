...
extension ICColorButton {
    ...
    //-------------- Background color -----------------
    public func setBackgroundColor(forICResName key:String, state: UIControlState) {
    ...
    //-------------- Border color -----------------
    public func setBorderColor(forICResName key:String, state: UIControlState) {
        self.ic_saveResKey(key, forHashKey: ICColorButton.ICBorderColorHashKey)
        self.setBorderColor(resGetColor(key), for: state)
    }
    
    //-------------- Tint color -----------------
    public func setTintColor(forICResName key:String, state: UIControlState) {
    ...
    override func didThemeChanged() {
        ...
        for (key, value) in self.ic_resStateHash(ICColorButton.ICBackgroundColorHashKey) {
            self.setBackgroundColor(resGetColor(value), for: UIControlState(rawValue: key))
        }
        
        for (key, value) in self.ic_resStateHash(ICColorButton.ICBorderColorHashKey) {
            self.setBorderColor(resGetColor(value), for: UIControlState(rawValue: key))
        }
        
        for (key, value) in self.ic_resStateHash(ICColorButton.ICTintColorHashKey) {
            self.setTintColor(resGetColor(value), for: UIControlState(rawValue: key))
        }
    }
}
